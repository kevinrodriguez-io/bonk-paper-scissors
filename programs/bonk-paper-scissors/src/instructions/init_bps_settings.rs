...
pub fn init_bps_settings(ctx: Context<InitBpsSettings>, time_for_penalization: i64) -> Result<()> {
    let bps_settings = &mut ctx.accounts.bps_settings;
    let bump = ctx.bumps.get("bps_settings").unwrap();
    bps_settings.set_inner(BpsSettings::new(*bump, time_for_penalization));
    Ok(())
}
